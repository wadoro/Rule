...
Install_ct(){
           ...
           `wget --no-check-certificate https://github.com/wadoro/Rule/releases/download/v"$ct_new_ver"/gost-linux-"$bit"-"$ct_new_ver".gz`
            ...
            `wget --no-check-certificate https://raw.githubusercontent.com/wadoro/Rule/master/gost/gost.service && chmod -R 777 gost.service && mv gost.service /usr/lib/systemd/system`
            `mkdir /etc/gost && wget --no-check-certificate https://raw.githubusercontent.com/wadoro/Rule/master/gost/config.json && mv config.json /etc/gost && chmod -R 777 /etc/gost`
